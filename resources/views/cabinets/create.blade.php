@extends('layouts.app')
@section('title', 'Nouveau cabinet médical')
@section('page-title', 'Nouveau cabinet médical')
...
<div class="max-w-xl">
    ...
    <form method="POST" action="{{ route('cabinets.store') }}" class="space-y-5">
        @csrf
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="ex. Cabinet 1 — Rez-de-chaussée"
            ...
            <div>
                ...
                <input type="text" name="description" value="{{ old('description') }}" placeholder="Courte description (facultatif)"
                       class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div>
                ...
                <select name="secretary_id" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    @foreach($secretaries as $secretary)
                        <option value="{{ $secretary->id }}" @selected(old('secretary_id') == $secretary->id)>
                            {{ $secretary->name }}
                        </option>
                    @endforeach
                </select>
